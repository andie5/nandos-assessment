...
// runCommands updates the rover position based on a list of string commands. Following a command, the rover position is checked to see if it is valid.
// If the rover move position is invalid at any point, the rover status is returned as is and no further moves are processed
func (rover *RoverStatus) runCommands(commands string, rovers map[int]RoverStatus, i int, grid PlanetAxis) {
...
func ProcessRovers(dataInput []string, grid PlanetAxis) map[int]RoverStatus {
	...
	for i := 1; i < len(dataInput); i++ {
		...
		if i%int(2) == 0 {
			...
			rover := rovers[i-1]
			rover.runCommands(row, rovers, i, grid)
		} else {
			var err error
			rovers[i], err = setStartingPosition(rowContents) // Rover id assigned is the row number of the data input where the starting position is provided
